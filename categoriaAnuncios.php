...
<?php
    ...
    if(isset($_REQUEST['cidade']) && isset($_REQUEST['estado'])){
        ...
        $anuncios = $model -> pesquisarAnunciosEstadocat($categoria,$cidade,$estado,$page);
        if($anuncios == null){
            $anuncios = $model -> pesquisarAnunciosEstadocat($categoria,$cidade,$estado,1);
        }
        $paginacao = $model -> paginacaoEstadoCategoria($estado,$cidade,$categoria);
        }else{
        $cidade = '';
        $estado = '';
        }
    ...
    if($anuncios == null){
        $anuncios = $model -> anunciosCategoria($categoria,$page);
        $paginacao = $model -> paginacaoCategoria($categoria);
    }
    ...
    if($anuncios == null){
        echo "<script>alert('Não existe anuncio dessa categoria');window.location.href='home.php';</script>";
    }
...
?>
        ...
        <div class="div1publi">
           <h1>Anúncios relacionados a trabalho do tipo <?php echo $categoria?></h1>
        </div>
    ...
    <?php
        include "PaginaçãoCategoria.php";
