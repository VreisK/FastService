...
<?php
    ...
    if(isset($_REQUEST['cidade']) && isset($_REQUEST['estado'])){
        ...
        $anuncios = $model -> pesquisarAnunciosEstado($pesquisa,$cidade,$estado,$page);
        if($anuncios == null){
            $anuncios = $model -> pesquisarAnunciosEstado($pesquisa,$cidade,$estado,1);
        }
        $paginacao = $model -> paginacaoEstadoPesquisa($pesquisa,$estado,$cidade);
        }else{
        $cidade = '';
        $estado = '';
        }
    ...
    if($anuncios == null){
        $anuncios = $model -> pesquisarAnuncios($pesquisa,$page);
        $paginacao = $model -> paginacaoPesquisa($pesquisa);    
    }
    ...
    if($anuncios == null){
        echo "<script>alert('Nenhum anuncio com essa descrição');</script>";
...
?>
        ...
        <div class="div1publi">
                ...
                <h1>Pesquisa por <?php echo $pesquisa ?></h1>
        </div>
